<template>
	<div class="container-fliud">
      ...
      <h5>Жорын ангилалууд ({{ categories.length }})</h5>

      <b-button pill variant="success" class="mt-3" to="/recipe-categories/add">
        Ангилал нэмэх
      </b-button>
          ...
          <tbody>
            <template v-if="!loading">
              <tr v-for="category in categories" :key="category.PK.S">
                ...
                <td>
                  <b-button variant="primary" size="sm" class="mb-2" :to="{ name: 'recipe-categories-id-edit', params: { id: category.PK.S } }"> Засах
                  </b-button>
                  <b-button variant="danger" size="sm" class="mb-2" @click="remove(category.title.S, category.PK.S)">
                    Устгах
                  </b-button>
                </td>
              </tr>  
    ...
    methods: {
      ...
      async deleteOk(bvModalEvt) {
        ...
        if (this.deleteCategory.pk) {
          ...
          try {
            let response = await this.$axios.delete("/recipe-categories?pk=" + this.deleteCategory.pk);
